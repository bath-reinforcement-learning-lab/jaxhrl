# MOC: Multi-option Option-Critic. Extends Option-Critic (Bacon et al. 2017)
# by updating every option's policy/value from each transition, not just the
# one that was actually active, using an arrival-probability weighting
# p_arrival(w|s) (the probability the agent would be running option w at s,
# given it entered s already committed to the option that was active) and a
# PPO-style clipped importance ratio to correct for the fact the action was
# actually sampled from the active option's policy, not w's.
import jax
...
def select_option_critic_action(key, obs, option, done, params, net, config):
    """Call-and-return, identical to option_critic.py: if the previously
    active option just terminated (or this is the first step of an
    episode), sample a new option epsilon-greedily over Q_Omega; otherwise
    keep it. Then sample an action from the (possibly-new) option's
    intra-option policy."""
...
def moc_loss_fn(params, target_params, net, batch, config):
    obs = batch.first["obs"]
    next_obs = batch.second["obs"]
    actions = batch.first["action"]
    active_options = batch.first["option"]
    old_logp_active = batch.first["logp"]
    ...
    q_w, beta_logits, action_logits = net.apply(params, obs)  # (B,O) (B,O) (B,O,A)
    beta = jax.nn.sigmoid(beta_logits)
    beta_active = beta[batch_idx, active_options]

    log_probs_all_options = jax.nn.log_softmax(action_logits, axis=-1)  # (B,O,A)
    action_idx = jnp.broadcast_to(actions[:, None, None], (B, num_options, 1))
    new_logp_all = jnp.take_along_axis(log_probs_all_options, action_idx, axis=-1)[:, :, 0]  # (B,O)
    probs_all = jnp.exp(log_probs_all_options)
    entropy_all = -jnp.sum(probs_all * log_probs_all_options, axis=-1)  # (B,O)

    # Bootstrap target for EVERY option, from the (Polyak-averaged) target net.
    ...
    v_next_target = jnp.max(next_q_target, axis=-1, keepdims=True)
    bootstrap_all = (1.0 - next_beta_target) * next_q_target + next_beta_target * v_next_target
    target_all = rewards[:, None] + config['gamma'] * (1.0 - dones[:, None]) * bootstrap_all
    target_all = jax.lax.stop_gradient(target_all)  # (B,O)

    advantage_all = jax.lax.stop_gradient(target_all - q_w)  # (B,O)

    # Importance ratio: probability the actually-taken action would have
    # been sampled under option w's CURRENT policy, vs. under the active
    # option's policy at collection time (the true behavior distribution).
    ratio_all = jnp.exp(new_logp_all - old_logp_active[:, None])
    clipped_all = jnp.clip(ratio_all, 1.0 - config['clip_eps'], 1.0 + config['clip_eps'])
    unclipped_obj = ratio_all * advantage_all
    clipped_obj = clipped_all * advantage_all
    per_option_actor_loss = -jnp.minimum(unclipped_obj, clipped_obj)  # (B,O)

    # Arrival probability: chance the agent is running option w at s, given
    # it entered s already committed to active_option.
    greedy_option = jnp.argmax(q_w, axis=-1)
    greedy_one_hot = jax.nn.one_hot(greedy_option, num_options)
    active_one_hot = jax.nn.one_hot(active_options, num_options)
    option_probs = config['epsilon'] / num_options + (1.0 - config['epsilon']) * greedy_one_hot
    p_arrival = beta_active[:, None] * option_probs + (1.0 - beta_active[:, None]) * active_one_hot
    p_arrival = jax.lax.stop_gradient(p_arrival)  # (B,O)

    actor_loss = jnp.mean(jnp.sum(p_arrival * per_option_actor_loss, axis=-1))
    critic_loss = jnp.mean(jnp.sum(p_arrival * (q_w - target_all) ** 2, axis=-1))
    entropy = jnp.mean(jnp.sum(p_arrival * entropy_all, axis=-1))

    # Termination gradient (Bacon et al., eq. 4) for the option that was
    # actually active, evaluated with the ONLINE network on s'.
    next_q_online, next_beta_logits_online, _ = net.apply(params, next_obs)
    beta_next_online = jax.nn.sigmoid(next_beta_logits_online)
    beta_next_active = beta_next_online[batch_idx, active_options]
    v_next_online = jnp.max(next_q_online, axis=-1)
    q_next_active_online = next_q_online[batch_idx, active_options]
    termination_advantage = jax.lax.stop_gradient(
        q_next_active_online - v_next_online + config['delib_cost']
    )
    nonterminal = 1.0 - dones
    termination_loss = jnp.mean(nonterminal * beta_next_active * termination_advantage)
...
if __name__ == "__main__":
    ...
    dummy_transition = {
        ...
        "logp": jnp.zeros((), dtype=jnp.float32),
    ...
    def run_chunk(carry, step_keys):
        ...
        def scan_step(c, rng):
            ...
            def do_train(operands):
                ...
                def loss(p):
                    return moc_loss_fn(p, target_params, net, batch, config)
    ...
    # Main Training Loop
    # -----------------------------------------------------
    print("Starting MOC parallel rollouts...")
