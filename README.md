...
| [HierQ](jaxhrl/HierQ.py) | Levy et al., *"Learning Multi-Level Hierarchies with Hindsight"* (Algorithm 2, Appendix) | Verified — every mechanism Algorithm 2 specifies holds in the tables it writes (20/20); hierarchical agents beat a flat agent by up to 6.3x fewer episodes, though the paper's 3-level > 2-level ordering does not reproduce |
