...
| [HiPPO](jaxhrl/HiPPO.py) | Li, Florensa, Clavera & Abbeel, *"Sub-Policy Adaptation for Hierarchical Reinforcement Learning"* (ICLR 2020) | Approximates policy gradient by assuming options are sufficiently diverse plus an unbiased latent-dependent
baseline. On-policy method to efficiently train all levels of the hierarchy jointly. Skill lengths are sampled from a cat distribution, no learnt termination function to improve robustness to distributional shift.|
| [HAC](jaxhrl/HAC.py) | Levy et al., *"Learning Multi-Level Hierarchies with Hindsight"* | Each level is trained as if the levels below were already optimal by relabelling the subgoal it proposed with the subgoal actually reached to allow all levels to learn in parallel without having a non stationary problem. Missed subgoals penalized sometimes so high level doesn't propose infeasible subgoals. Has a recursive multilevel skill hierarchy.|
