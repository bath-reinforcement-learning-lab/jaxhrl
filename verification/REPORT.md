...
`results/hdqn_vs_flat_success.png` shows the shape that matters more than the
absolute numbers: h-DQN's rolling success rate climbs steadily after ~episode
1300 (peaking near 19%), while the flat baseline trends toward zero over the
same window — it's not just slower, it's failing to consolidate on this task
at all, exactly the qualitative gap (flat DQN plateaus/collapses on the
delayed-reward chain while h-DQN's subgoal-conditioned intrinsic reward lets
it keep improving) that the paper's Figure 3 reports. Neither agent is
anywhere near converged at 2,500 episodes / no tuning — this budget is enough
to see the qualitative separation the paper predicts, not to claim the
...
We got the paper's actual PDF (arXiv 2106.13105) to pin down the real
experimental setup rather than guessing, and built `foragingworld.py` to
match the paper's own "Foraging World" domain as closely as practical: `m=2`
...
### A bug we found and fixed -- in our own test harness, not the repo

The first large-scale attempt (256 parallel envs, 46M env-steps) came back
*worse* than an earlier, much smaller 220k-step pass: SF-prediction accuracy
went to near-zero/negative correlation and GPI lost to the single-option
baseline on both novel weight vectors, one of them with negative reward. More
data making things worse is a red flag, so we didn't stop there. Instrumenting
the training loop to log each food type's visitation rate over time showed
why: the pretraining behavior policy was epsilon-greedy over **GPI's own
combined policy for `w_explore=(1,1)`** ("want both nutrients"), and since
food type `y3=(1,1)` uniquely maximizes that objective, the greedy component
converges to camping almost exclusively at `y3`. Over the course of that run,
`y1`/`y2` visitation rate collapsed from ~2% to under 0.3% while `y3` climbed
to 35% — starving the exact two cumulants (`y1`, `y2`) that GPI needs good
data on, of data, and explaining why *more* training made the successor
features (and everything downstream) actively worse rather than better. This
is a coverage-collapse bug in **our verification script's exploration design**
(`okeyboard_verify.py`), confirmed and fixed by switching to pure
uniform-random exploration throughout training (`eps_min=1.0`) — re-running
the identical 60k-macro-step config with only that change kept all three food
types' hit rates flat at ~2% for the entire run, with zero drift. It is not a
finding about `sf_loss`, `Agent`, or GPI in the repo itself.
...
Absolute performance in all cases stays well below the theoretical optimum
(e.g. 0.41-0.92 vs. 1.0-2.0) -- the independent SF-accuracy check
(`results/ok_sf_accuracy.png`, r=0.43/0.36) shows the successor features
still carry real signal but aren't highly precise. Worth flagging a
methodological limitation of that particular check rather than treating it as
damning: it compares `psi` (an off-policy, "if I followed this option's own
best policy from here" estimate) against the empirical return actually
observed along trajectories generated by the *combined* `w=(1,1)` exploration
policy, which is a systematically different quantity, not a clean apples-to-
apples target -- the task-reward comparison above is the more direct test of
...
- Small-scale, single-CPU, no hyperparameter search — these are cheap sanity
  checks, not full reproductions. The DCEO result above should still be
  re-validated at the repo's intended scale (parallel envs, full training
  budget) before trusting it in production runs.
- A DCEO diagnostic (option behavior against an oracle/ground-truth
  representation, in `results/dceo_verification_summary.json`) surfaced a
  weak/inconclusive signal that we did **not** chase down — it may be nothing
  more than this test's small training budget, but it wasn't ruled out either.
- `jaxhrl/h-DQN.py` and `jaxhrl/option_keyboard.py` both import
  `jaxhrl.common.jax_wrappers`, a module that doesn't exist anywhere in the
  repo (only `jaxhrl.common.wrappers` does) — so as of this commit, neither
  script can actually be run end-to-end via `python jaxhrl/h-DQN.py --config
  ...`. That's unrelated to the paper-fidelity findings above (we sidestepped
  it via `repo_loader.py` to import just the network/loss code), but it'll
  block real training runs until fixed.
